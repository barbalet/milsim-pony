...
The active direction for the project is now the Cycle `117`-`196` fully playable game plan in [Docs/DEVELOPMENT_BACKLOG.md](/Users/barbalet/github/milsim-pony/Docs/DEVELOPMENT_BACKLOG.md). Cycle `122` is the current development line: Cycle `117` added the formal profiling baseline, and Cycles `118`-`122` add live vegetation friction/masking, difficulty regression telemetry, water closeout validation, route-author LOS overlay telemetry, and conditional mission scripting with core-backed route failure. The remaining cycles complete REVIEW2, the added playability recommendations, and outstanding release hardening: all-route minimap/audio validation, notarized packaging, SSAO, CSM, procedural foliage, route tutorial, simplified HUD, sniper kill objectives, basic AAR, performance presets, gameplay harnesses, renderer modernizations, weapon animation, advanced observer tactics, one polished recommended route campaign, tester feedback, and a notarized playable game candidate by Cycle `196`.
