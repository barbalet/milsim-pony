...
The current default build launches into **Canberra Restore Cleanup Preview Validation**, a cycle `70` pass that keeps the Woden-to-Belconnen route, paired-observer baseline, observer-audio feedback, mission-script hooks, surface-fidelity closeout, review-resume card, persistence guardrails, restore-target preview, restore-readiness report, manual-restore arming, restore-prompt contract, restore-execution gate, restore-audit trail, restore-freshness policy, and restore-retention policy locked while adding a restore-cleanup preview report. The route is designed to prove that the world remains readable at distance through a 4x scope while the HUD, scoped reticle, overhead map, cover cues, observer audio, mission phase readouts, active-route binding status, alternate-route preview path, staged-route distance and footprint, binding-gate status, selection-lock rule, activation-guard rule, rollback-guard rule, commit-gate rule, dry-run rule, promotion-readiness rule, promotion-audit rule, restart-boundary rule, handoff-arming rule, handoff-confirmation rule, release-gate rule, live-switch preflight rule, collision-authoring inventory, environmental-motion status, surface-fidelity status, session-persistence status, restore-target preview, restore-readiness report, manual-restore arming, restore-prompt contract, restore-execution gate, restore-audit trail, restore-freshness policy, restore-retention policy, restore-cleanup preview, restart-safe handoff rule, selection rules, and checkpoint ownership now agree on route expansion without losing the named road and district read.
