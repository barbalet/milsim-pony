...
The current default build launches into **Canberra Lighting Architecture Decision**, a cycle `98` pass that keeps the Woden-to-Belconnen route, scoped rifle presentation, observer-audio feedback, mission-script hooks, surface-fidelity closeout, alternate-route live binding, active-route map accuracy, Black Mountain/Telstra/Bruce material assignments, West Basin/Yarralumla material closeout, the third authored rehearsal route, overhead-map collision blocker footprints, guarded manual checkpoint restore, practical scope calibration, vegetation concealment/traversal feedback, CSM readiness profiling, distant LOD metadata, water reflection-probe evidence, repeatable packaging validation, and tester delivery locked while adding a time-of-day and lighting architecture decision. The route is designed to prove that the world remains readable at distance through a 4x scope while the HUD, scoped reticle, overhead map, cover cues, observer audio, mission phase readouts, patrol pair state, LOS debug state, scan-halt-resume state, world audio state, route-selection state, `Map Accuracy:` footer, `Collision Preview:` footer, `Shadow Profile:` footer, `Distant LOD:` footer, `Water Reflection:` footer, `Packaging:` footer, `Tester Delivery:` footer, `Lighting Plan:` footer, material footers, weapon status, profiling baseline, collision-authoring inventory, environmental-motion status, surface-fidelity status, session-persistence status, `Restore Execution Design:`, `Restore Safety Checks:`, `Session Audio:`, `Scope Calibration:`, `Vegetation Concealment:`, `CSM Profile:`, `LOD Reflection:`, restart-safe handoff rule, selection rules, and checkpoint ownership now agree on route expansion, optic usability, concealment readability, renderer profiling, LOD planning, reflection evidence, packaging readiness, tester handoff, lighting architecture, and restore safety.
...
- Confirm that scoped rendering remains stable across distant terrain and skyline silhouettes while `Scope Calibration:` reports practical range, drop, hold, parallax, and edge stability cues, `CSM Profile:` reports the current shadow-map baseline plus planned cascade split targets, `LOD Reflection:` reports landmark impostor targets plus water reflection-probe evidence, `Packaging:` reports the release version policy, archive naming rule, manifest checks, `Tester Delivery:` reports the tester channel, notarization status, CI plan, delivery checklist, and `Lighting Plan:` reports the fixed daylight scenario plus clustered-lighting/render-graph decisions.
...
The active direction for the project is basin-scale Canberra coverage from Woden to Belconnen, with Lake Burley Griffin as a readable anchor, a district street atlas in the overhead map, and the 4x scope as the core validation tool. The live build is now on cycle `98`, using the paired-observer route as the stable baseline while the selected alternate route can be armed from briefing and rebound as the active checkpoint sequence before live movement begins. Manual restore can execute only after the stored card passes identity, freshness, and target checks and the user reviews then explicitly executes the restore target; the current lighting pass keeps the renderer route on a fixed single-sun daylight scenario while documenting the measured gates for future time-of-day expansion, clustered/Forward+ lighting, and render-graph modernization.
