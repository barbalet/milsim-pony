...
The active direction for the project is now the Cycle `117`-`196` fully playable game plan in [Docs/DEVELOPMENT_BACKLOG.md](/Users/barbalet/github/milsim-pony/Docs/DEVELOPMENT_BACKLOG.md). Cycle `116` is the current live build, with automated capture, distant-building LOD implementation, configurable scenario time-of-day lighting, a Forward+ lighting start, scoped-safe anti-aliasing, a physical atmosphere baseline, fallback-safe shadow rendering, scalable SDF-style UI text, bounded SSR/IBL reflections, render graph scaffolding, authored audio mix controls, player-facing firing feedback, live group NPC patrol behavior, Black Mountain/West Basin source-backed texture coverage, multi-route playability, scope optics, collision authoring workflow, and save/resume closeout complete for their canonical review scope. The next eighty cycles complete REVIEW2, the added playability recommendations, and outstanding release hardening: formal profiling, vegetation interaction, water, mission scripting, all-route minimap/audio validation, notarized packaging, SSAO, CSM, procedural foliage, route tutorial, simplified HUD, sniper kill objectives, basic AAR, performance presets, gameplay harnesses, renderer modernizations, weapon animation, advanced observer tactics, one polished recommended route campaign, tester feedback, and a notarized playable game candidate by Cycle `196`.
